describe("main platform", () => {
  it("opens", () => {
    ...
    cy.exec("pwd").then((result) => {
      ...
      cy.get('[placeholder="/Users/sweep/path/to/repo"]')
        .clear()
        .type(sections.slice(0, sections.length - 1).join("/"));
      cy.get('[placeholder="node_modules, .log, build"]')
        .clear()
        .type(".git, node_modules, venv, __pycache__, .next, cache, logs");
      cy.get("#name")
        .clear()
        .type(sections.slice(0, sections.length - 1).join("/"));
      ...
      cy.get("#creation-panel-plus-sign-wraper > .flex > .inline-flex").trigger(
        "mouseover",
      );
      ...
      cy.get(
        '[style="flex: 68.2 1 0px; overflow: hidden;"] > .h-full > .mt-2',
      ).click();

      console.log("Checking if the right changes were made");
      console.warn(
        "This test is not working as expected. It should be checking if the changes were made to the file, but it's not.",
      );
