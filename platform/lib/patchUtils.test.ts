...
describe("patchUtils", () => {
  ...
  describe("softIndentationCheck", () => {
    ...
    test("should return original code blocks if new code is indented but old code is not found", () => {
      const fileContentsWithIndent =
        "    some line\n    another line\n    a third line";
      const [newOldCode, newNewCode] = softIndentationCheck(
        "wrong indentation\nanother line",
        newCode,
        fileContentsWithIndent,
      );
    ...
    test("should handle old code with mixed indentation levels", () => {
      const mixedIndentFileContents =
        "  some line\n    another line\n  a third line";
      const mixedIndentOldCode = "some line\n    another line";

      const [newOldCodeMixedIndent, newNewCodeMixedIndent] =
        softIndentationCheck(
          mixedIndentOldCode,
          newCode,
          mixedIndentFileContents,
        );
...
`;
    ...
    test("should return updated file contents and an empty error message when modification is applicable", () => {
      const [updatedFileContents, errorMessage] = parseRegexFromOpenAIModify(
        response,
        fileContents,
      );
...
>>>>>>> MODIFIED
`;
      const fileContentsMultiple =
        "first line\nold content\nmiddle line\nadditional old content\nlast line";
      const [updatedFileContents, errorMessage] = parseRegexFromOpenAIModify(
        responseMultiple,
        fileContentsMultiple,
      );
      expect(updatedFileContents).toBe(
        "first line\nnew content\nmiddle line\nadditional new content\nlast line",
      );
      expect(errorMessage).toBe("");
    });
...
`;
      const [newFileContentsWithEscapedChars, errorMessageWithEscapedChars] =
        parseRegexFromOpenAICreate(responseWithEscapedChars, fileContents);
      expect(newFileContentsWithEscapedChars).toEqual(
        "This is the new file content with \\n new line and \\t tab characters.\n",
      );
...
`;
      const fileContentsToBeReplaced =
        "All of this will be replaced.\nEvery single line.";
      const [newFileContentsReplaced, errorMessageReplaced] =
        parseRegexFromOpenAICreate(
          responseReplaceEntireContent,
          fileContentsToBeReplaced,
        );
    ...
    test("should return new file contents when a new file creation hunk is provided", () => {
      const [newFileContents, errorMessage] = parseRegexFromOpenAICreate(
        response,
        fileContents,
      );
