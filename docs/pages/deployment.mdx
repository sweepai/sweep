...
WHITELISTED_USERS=your-username,other-username,another-username
```
If Probot fails, please follow the manual instructions above.
</Tabs.Tab>
<Tabs.Tab>
...
Please create an (OpenAI API key)[https://platform.openai.com/account/api-keys] and add it to your .env.
```sh filename=".env"
APP_ID=123456
PRIVATE_KEY="-----BEGIN RSA PRIVATE KEY-----\n...\n-----END RSA PRIVATE KEY-----\n"
WEBHOOK_SECRET=213921898f9as8f92139128989f
...
Now to host Sweep you need to run the Sweep webhook on a publicly accessible server. We recommend using a cloud provider like DigitalOcean, AWS, or Azure, but you can also run it locally and use a reverse proxy like Ngrok. For this guide, we will use DigitalOcean.
...
```sh filename="terminal"
docker compose up --build hosted # Add -d to run in background
```
