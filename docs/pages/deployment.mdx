...
_🎉 We recently changed our license to the Elastic License V2 to allow Sweep for commercial usage._
...
- Add your GitHub App name to the secrets as `GITHUB_BOT_USERNAME`
  If you would like to set it up yourself, scroll down to the Manual GitHub App Setup section.
...
_Starting here it gets more involved so if you run into issues, please reach out to us on [Discord](https://discord.gg/sweep) or open a GitHub issue (ironic, we know!) at https://github.com/sweepai/sweep/issues. We are online, please ping us @kevin, @william, and @luke._

<details>
<summary><h2>Deploy on DigitalOcean</h2></summary>
...
<p style={{ marginTop: 16, marginBottom: 32 }}>
  <a href="https://cloud.digitalocean.com/apps/new?repo=https://github.com/sweepai/sweep/tree/main">
    <img
      src="https://www.deploytodo.com/do-btn-blue-ghost.svg"
      alt="Deploy to DO"
    />
  </a>
</p>
...
<details>
<summary><h2>Deploy on AWS/GCP/Azure Instance</h2></summary>

If you would like to deploy on AWS/GCP/Azure, you can do so by following the steps below.

<details>
<summary><h3>1. Create Instance</h3></summary>
Recommended Specs:
- OS: Ubuntu
- RAM: 8 GB
- CPU: 4 vCPUs
...
<details>
<summary><h3>2. Install Docker</h3></summary>
```sh
...
<details>
<summary><h3>3. Pull Image</h3></summary>
```sh
...
<details>
<summary><h3>4. Create .env File</h3></summary>
```sh
touch .env
```

Then, using your favorite text editor, add the following to the `.env` file from the previous step:
```yaml
# These two are from step 1. Rename PRIVATE_KEY to GITHUB_APP_PEM
APP_ID=1234
GITHUB_APP_PEM="-----BEGIN RSA PRIVATE KEY-----\n...\n-----END RSA PRIVATE KEY-----\n"
...
<details>
<summary><h3>5. Launch Docker Image</h3></summary>
```sh
...
<details>
<summary><h3>6. Update GitHub Webhook URL</h3></summary>
...
If you were unable to set up the GitHub App using the above steps, you can follow the manual setup below.

<details>
<summary><h2>Alternative to 1: Manual GitHub App Setup</h2></summary>

<i>
  Only follow this section if you were unable to set up the GitHub App using the
  above steps.
</i>
...
_There's additional configurable settings like Redis caching and GPU-accelerated embeddings that we use for our hosted product that we can document further for self-hosting. If this interests you please contact us at https://discord.gg/sweep._
