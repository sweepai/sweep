...
Hacker News link: https://news.ycombinator.com/item?id=36948403. Please leave a comment if you enjoy this blog post or have any questions!
...
Our current code chunking algorithm chunks **2M+ files a day** and is [open-sourced](https://github.com/sweepai/sweep/blob/b267b613d4c706eaf959fe6789f11e9a856521d1/sweepai/utils/utils.py#L48-L126)!
...
This algorithm is currently embedded into our codebase but can be open-sourced as a standalone project or as part of Langchain. Although we lack the time to undertake this task, we are more than willing to help anyone interested in implementing it. Feel free to leave a comment or reach out to me on the HN post if interested at https://news.ycombinator.com/item?id=36948403.

Another problem is that code far apart may still need the context of each other. For example, a class method may need the context of the class header and long functions also need their function signatures. A possible improvement would be to somehow use a format like:
