...
Standard vector databases worked great for Sweep when we had a single index, but they became a pain as we started managing multiple indices, frequent updates, and self-hosting. We started with Chroma as our vector database but decided to go with something we built ourselves.

Our backend consists of a single docker image that handles vector search over hundreds of repositories, each with a few thousand files.
...
We have a couple of configuration options:
...
This is our main loop. It turns a batch of texts into embeddings, handling cache reads and writes as it goes.
```python
...
This makes a lot of sense if we have small amount of very large indices, but becomes cumbersome because we have many small indices. We’d also have to manage the reindexing as each user pushes code to their repos (multiple times an hour).
...
#### Non-requirements
- This approach has high latency.
