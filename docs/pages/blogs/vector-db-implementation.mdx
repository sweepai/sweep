...
Standard vector databases worked great for Sweep when we had a single index, but they became a pain as we started managing multiple indices, frequent updates, and self-hosting. Our backend consists of a single docker image that handles vector search over hundreds of repositories, each with a few thousand files.
