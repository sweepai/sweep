# Language models are bad at refactoring code
...
When extracting a function out of a larger function or class, you’ll have to:

- Gather the correct input variables
- Make sure the state is returned correctly
- Handle any side effects

We’ve been looking for the best use cases to focus on for Sweep, and one of these has been writing and executing unit tests. *This article is about refactoring, but don’t worry, we didn’t forget about it ;)*.

Unit tests have the most value for code that changes a lot. We decided that we’d set out on a simple task for the Halloween weekend: get Sweep to refactor our own code.


# Lanugage models

If you’ve read our previous blog on why modifying code with GPT4 is hard, https://docs.sweep.dev/blogs/gpt-4-modification you’ll know that a refactor is even harder. GPT4 has to perfectly extract all of the inputs to a 50-75 line snippet of code within a larger snippet (hundreds of lines?).
...
We wanted something deterministic, so we used Rope. Rope is a refactoring library for Python that uses the Python AST, allowing for refactors that work 100% of the time.
...
In our own code, we added a section that makes the score agnostic to these lines.
Here's a snippet:
```
elif (
            t_line.strip() == ""
...
Another interesting problem we had to deal with was that GPT4 thinks it must extract the new code and generate the function at the same time. This is an issue with the underlying model that we discovered. 

As a toy example, if it was extracting
```
print(x)
x += 1
```

from a larger section of code, it would generate
```
def (x):
    print(x)
    x += 1
...
even though Rope will later generate this section and replace all occurrences. We had to add a special case to our scoring algorithm to handle this, which backoffs by first deleting the first line, then the last line.
...
When it generates >400 lines of code to extract, it risks running past the token limit (I saw it generate 4.9k tokens for one of our files), or even forgetting to close the `</extraction>` XML tag.

One trick I found helps with this is to add a sort of "anchor" to the chain-of-thought(CoT) prompting. Instead of the classic "Think step-by-step to solve the problem",
...
```
This helps *a lot*. Our hypothesis is that when starting to extract the span, the transformer(underlying model architecture of GPT4) can attend to and anchor the generation to the original start `documents = []`, and then finish extracting by doing the same for `ids.append(f"{gh_file_path}:{snippet.start}:{snippet.end}")`.

We added this section to almost all of our prompts, and it made them much more reliable. Task specific CoT with "anchors" seems to be the way to go!

## Final result

Finally, we can have GPT4 generate multiple spans to refactor, as well as the function names for these methods. We can pass Rope the extracted span (using GPT4 to make sure the span corresponds to the user request) and the AI generated function name to perfectly extract the code.
