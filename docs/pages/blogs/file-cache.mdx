...
## Background
We spend a lot of time on prompt engineering our agents at Sweep. Our agents take a set of input strings, format them as a prompt, then extract the important output from them. We chain multiple agents to turn a GitHub issue to a pull request.
For example, to modify code we'll input the old code, relevant context, and instructions then output the new code.
...
LLM calls take a really long time, but their inputs and outputs are easy to cache and caching saves a massive amount of time. We can use the input prompt/string as the cache key, and the output string as the cache value.
...
2. lru_cache doesn't support ignoring arguments that invalidate the cache. Sometimes we'll pass in our custom `chat_logger` which stores the chats for visualization. It contains the current timestamp, `chat_logger.expiration`, which invalidates the cache when it's serialized.
    - To counteract this we added ignored params, used like so: `file_cache(ignore_params=["chat_logger"])`. This removes `chat_logger` from the cache key construction and prevents bad invalidation.
...
#### cache key creation / invalidation conditions
...
To handle 2. we used `inspect.getsource(func)` to add the function's source code to the hash, invalidating the cache when the the code changes.
