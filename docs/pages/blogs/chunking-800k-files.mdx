# Chunking 800k+ files/day for Code Search using Concrete Syntax Trees

Author: Kevin Lu

Initializing any vector store requires chunking large documents for effective search.

Why can’t we just embed entire files? Envision our main API [endpoint](https://github.com/sweepai/sweep/blob/b267b613d4c706eaf959fe6789f11e9a856521d1/sweepai/api.py) as a file:
...
If I search “GitHub Action run”, it should match the section corresponding to the [switch case block](https://github.com/sweepai/sweep/blob/b267b613d4c706eaf959fe6789f11e9a856521d1/sweepai/api.py#L295-L313) checking for the “check_runs completed” event. However, this is 20 lines of code out of 400+ lines so even a perfect search algorithm would only say that this is a 5% match. But if we chunked the 400 lines into 20 chunks of 20 lines, this would match the correct switch case block.

But how do we get meaningful 20-line chunks? Naively we could break up the 400-line file evenly into 20-line chunks. But this would yield terrible results. Semantically similar code would not stay together and context would be lost. For example, function headers would be separated from their implementation details and the docstrings.

Our current code chunking algorithm chunks **800k+ files a day**, which is [open-sourced](https://github.com/sweepai/sweep/blob/b267b613d4c706eaf959fe6789f11e9a856521d1/sweepai/utils/utils.py#L48-L126)!

## Constraints 🚧

Generally, most are capped by token count, since it makes estimating the number of chunks to fetch easy to compute. For simplicity, we decided to use character count, with a max character count of 1500 This is because the average token to character ratio for code is about 1:5 and we want to be on the safer side. The problem becomes getting as close to 1500 characters as possible, while still ensuring the chunks are semantically similar and all relevant contexts are unseparated.
...
This algorithm is currently embedded into our codebase but can be open-sourced as a standalone project or as part of Langchain.

A possible improvement is to somehow also include the class headers in a class method that’s deep into the code.
