...
GPT4 can generate unit tests, refactor a function, and add comments/docstrings for your code. You can even use GPT4 with a 32k context window to write an entire app from scratch.

However, to enable GPT4 to plan code changes at the repository level, such as when solving Jira tickets, we need to undertake significant work. We need to turn the user input from an issue title and description into a set of instructions, and then translate those to code changes. It should look like this:
<br/>
...
This must include a powerful search engine, but search is not sufficient. Relying solely on RAG (Retrieval-Augmented-Generation) requires a significant amount of luck to retrieve the files we need.
...
This is a prevalent effect known as lost-in-the-middle (LIM), where Language Models (LLMs) only focus on the context at the beginning and end of the prompt. Thus, when we try to add more context, GPT4 will also ignore a lot of it.
...
We pruned the context by instructing GPT4 to identify elements it wants to remove, such as certain snippets or paths/directories in the repository that are clearly not needed. We started by fetching everything, and only have GPT4 name what it wants to remove.

This process results in a comprehensive list of all irrelevant elements, and we hope that GPT4 doesn't mistakenly identify and remove any necessary components. The entire pipeline would fail when GPT4 pruned something we needed, or forgot to prune useless files.
...
A more reliable approach would be to gradually iterate until we reach an optimal state. In the above, we can use GPT4 as a context optimizer. If it sees the state of the repo and decides no changes are needed, i.e. keep everything shown and don't add anything new, we’re at a local maxima.

It appears that the model's decision-making quality peaks when the context window is between 10k and 15k tokens. So we don’t want to make decisions at 20k. Rather we’d make the decision at 10k to both prune and expand the current context, exploring more files and directories, and removing unexplored ones.

This approach enables us to remain within our context limit while still iterating towards the previously mentioned local maximum.
