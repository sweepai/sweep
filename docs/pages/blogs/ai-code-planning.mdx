# **🗺️ How AI Plans Code Changes Across an Entire GitHub Repository**

**William Zeng** - October 9th, 2023

---

GPT-4 can generate unit tests, refactor functions, and add comments/docstrings to your code. You can even use GPT-4 with a 32k context window to create an entire app from scratch.

However, when it comes to planning code changes at the repository level, such as solving Jira tickets, more work is needed. We have to transform user input from an issue title and description into a set of instructions and then translate those into code changes. It should follow a process like this:
<br/>
...
This process requires a powerful search engine, but search alone is not enough. Relying solely on Retrieval-Augmented-Generation (RAG) often relies on luck to locate the necessary files.

## Lexical search and context pruning

Efforts were made to manipulate and enhance the input context, but encountered issues when additional inputs were added. This resulted in short-term performance degradation, as GPT-4's performance (planning, coding, and instruction following) tends to decline after approximately ~20k tokens.

This phenomenon, known as "lost-in-the-middle" (LIM), is prevalent, where large language models (LLMs) focus primarily on the context at the beginning and end of the prompt. When more context is added, GPT-4 often ignores a significant portion of it.

### 1. Pruning the Input Context (Repository Tree and Files)

One approach was to use the repository tree as an input context. Initially, all directories containing files fetched from the hybrid search index were included, but this was insufficient.
