...
In this article, we’ll explore our planning engine for Sweep, but these two parts go hand in hand. As we'll discuss later, planning doesn’t end once we start executing and in fact should be dynamic.
...
Here are some ideas that we tried previously. We encountered an interesting issue everytime we added additional inputs. This will hurt our performance in the short term because of how GPT4 currently degrades in performance (both planning, coding, and instruction following) after ~20k tokens.

This is a prevalent effect known as lost-in-the-middle (LIM) where LLMs only really pay attention to context at the beginning and end of the prompt. Thus, when we try to add more context, GPT4 will also ignore a lot of it.

### 1. Prune the Input Context(repository tree and files)

One of the input contexts we tried was the repository tree. We already only expand directories that are related to files that hit the hybrid search index, but even this is not nearly enough.
...
We prune the context by having GPT4 name what it wants to remove (say some snippets or paths/directories in the repo) that we clearly don’t need. We started by fetching everything, and only have GPT4 name what it wants to remove.
...
Something else that allows us to start at favorable initial conditions on top of our previous vector search, was lexical search. I looked around and tried to find a nice python library for TF-IDF, and I landed on [whoosh](https://whoosh.readthedocs.io/en/latest/). This library is unmaintained and unfortunately caused several deadlocks when we ran this in production with multiple threads trying to access the same cache, even if we set this to cache in memory instead of disk.
...
N-grams (another historical trick) also helps here. For context, TF-IDF leverages the inverse frequency of a term to boost the ranking. An N-gram captures patterns like `vector_db.search` or `logger.error(")`, giving us more context than just single tokens.

We also tokenized on pascal-case, camel-case, and snake-case. This means that `ChatGPT`, `chat_gpt`, and `chatGPT` all become `chat` and `GPT`.  This also allows us to heavily match any code that is copied verbatim as well as class definitions and instantiations.
...
{/* I don't understand this sentence tbh */}
