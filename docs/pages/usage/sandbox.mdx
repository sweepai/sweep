...
To ensure that your sandbox is set up properly, you can test it by creating an issue with a title of the format "Sweep (sandbox): FILE_PATH", where FILE_PATH is the path to some file you want to run the sandbox on.

For example, we can test the sandbox on `sweepai/api.py` by titling the issue "Sweep (sandbox): sweepai/api.py" at https://github.com/sweepai/sweep/issues/2371.
