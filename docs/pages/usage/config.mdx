...
Ex: if you usually have to clean up logs, write `"Python error logs should use loguru. If they are under an exception, they should use traceback."`

##### 2. Rules should be specific in the type of change:

Ex: `"Update the README.md file whenever a new feature is added."` Instead, say `"Update the README.md file whenever a new feature is added. Make sure to include a description of the feature and any new commands."`

##### 3. Tell Sweep where the new code should go:

Ex: `"We use unit tests for our code. If we have "class.py", we should have "class_test.py" in the same directory."`
