...
try:
    ...
    if comment.strip().upper() == "REVERT":
        if pr_path is None:
            logger.error("Cannot revert file because the comment was not made on a specific file.")
            return {"success": False, "error": "Cannot revert file because the comment was not made on a specific file."}
        sweep_bot.rollback_file(repo, pr_path, branch_name)
        return {"success": True}
