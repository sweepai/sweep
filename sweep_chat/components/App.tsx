...
const SuggestedChangeDisplay = (
{
  ...
  const [splitDiff, setSplitDiff] = useState(
    suggestion.originalCode.length > 0 ? (suggestion.newCode.length / suggestion.originalCode.length < 3) : false
  )
  return (
    <div className="fit-content mb-6" key={index}>
      ...
      >
        <code>
          {collapsed ? (
            <FaChevronRight onClick={() => setCollapsed(false)} className='hover:cursor-pointer inline mr-2' style={{marginTop: -2}} />
          ) : (
            <FaChevronDown onClick={() => setCollapsed(true)} className='hover:cursor-pointer inline mr-2' style={{marginTop: -2}} />
          )}
          {suggestion.filePath}{' '}
          {suggestion.state == 'pending' ? (
            '(pending)'
...
function App({ defaultMessageId = '' }: { defaultMessageId?: string }) {
  ...
  const [suggestedChanges, setSuggestedChanges] = useState<
    StatefulCodeSuggestion[]
  >([])
  const [suggestedChangesCollapsedArray, setSuggestedChangesCollapsedArray] = useState<
    boolean[]
  >([])
  const [appliedChanges, setAppliedChanges] = useState<
    StatefulCodeSuggestion[]
  >([])
  const [appliedChangesCollapsedArray, setAppliedChangesCollapsedArray] = useState<
    boolean[]
  >([])
  ...
  useEffect(() => {
    setCurrentTab(suggestedChanges.length === 0 ? 'context' : 'changes')
  ...
  useEffect(() => {
    if (suggestedChanges.length !== suggestedChangesCollapsedArray.length) {
      setSuggestedChangesCollapsedArray(
        suggestedChanges.map((_, index) => 
          index < suggestedChangesCollapsedArray.length 
            ? suggestedChangesCollapsedArray[index] 
            : false
        )
      )
    }
  ...
  useEffect(() => {
    if (appliedChanges.length !== appliedChangesCollapsedArray.length) {
      setAppliedChangesCollapsedArray(
        appliedChanges.map((_, index) => index < appliedChangesCollapsedArray.length ? appliedChangesCollapsedArray[index] : false)
      )
    }
      ...
      try {
        ...
        if (!featureBranch || !pullRequestTitle || !pullRequestBody) {
          const prMetadata = await authorizedFetch(
            ...
            {
              repo_name: repoName,
              modify_files_dict: currentCodeSuggestions.reduce((
                    acc: Record<
                      string,
        ...
        {(repoNameValid || messagesId) && (
          <ResizablePanelGroup direction="horizontal">
            <ResizablePanel defaultSize={33} className="pr-4">
              <Tabs
                value={currentTab}
                onValueChange={(value) => setCurrentTab(value as 'changes' | 'context')}
              ...
              >
                ...
                <TabsContent value="changes" className='grow overflow-y-auto'>
                  <ScrollArea className='overflow-y-auto pr-4'>
                    <div className="flex justify-between mb-4 align-start">
                      <div className="flex items-center align-middle pr-2">
                        <NavigationMenu>
                          <NavigationMenuList>
                            <NavigationMenuItem>
                        ...
                        {isStream.current ? (
                          <Button className="text-zinc-400 bg-transparent hover:drop-shadow-md hover:bg-initial hover:text-zinc-300 rounded-full px-2 mt-0"
                            onClick={() => {
                              isStream.current = false
                            }}
                            aria-label="Stop"
                    ...
                    >
                      {suggestedChanges.length > 0 && (
                        <h2 className='font-bold mb-4'>Selected Changes</h2>
                      )}
                      {suggestedChanges.map((suggestion, index) => (
                        <SuggestedChangeDisplay
                          ...
                          setSuggestedChanges={setSuggestedChanges}
                          key={index}
                          collapsed={suggestedChangesCollapsedArray[index]}
                          setCollapsed={(collapsed) => {
                            setSuggestedChangesCollapsedArray(suggestedChangesCollapsedArray.map((_, i) => i === index ? collapsed : suggestedChangesCollapsedArray[i]))
                          }}
                        />
                      ))}
                      {appliedChanges.length > 0 && (
                        <h2 className='font-bold mb-4'>Applied Changes</h2>
                      )}
                      {appliedChanges.map((suggestion, index) => (
                        <SuggestedChangeDisplay
                          suggestion={suggestion}
                          index={index}
                          setSuggestedChanges={setAppliedChanges}
                          key={index}
                          collapsed={appliedChangesCollapsedArray[index]}
                          setCollapsed={(collapsed) => {
                            setSuggestedChangesCollapsedArray(suggestedChangesCollapsedArray.map((_, i) => i === index ? collapsed : suggestedChangesCollapsedArray[i]))
                          }}
                        />
