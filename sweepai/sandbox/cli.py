...
def copy_to(container):
    try:
        git_ignore_patterns = [line.strip('/') for line in open('.gitignore').read().splitlines() if not line.startswith("#") and line.strip()]
    except FileNotFoundError:
        git_ignore_patterns = []
    git_ignore_patterns += [".git"]
    spec = pathspec.PathSpec.from_lines(pathspec.patterns.GitWildMatchPattern, open(".gitignore").read().splitlines())
    files_to_copy = {f for f in tqdm(Path(".").rglob("*"), desc="Getting files to copy") if f.is_file() and not spec.match_file(f)}
