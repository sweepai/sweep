...
def get_deeplake_vs_from_repo(
    ...
    chunk_size = len(documents) // os.cpu_count()
    document_chunks = [documents[i:i + chunk_size] for i in range(0, len(documents), chunk_size)]

    # Generate embeddings for each chunk in a separate process
    with Pool() as pool:
        embeddings_chunks = pool.map(compute_embeddings, document_chunks)

    # Combine the embeddings generated by each process into a single list of embeddings
    embeddings = [embedding for chunk in embeddings_chunks for embedding in chunk]
