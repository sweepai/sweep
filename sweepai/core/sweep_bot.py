...
from sweepai.core.prompts import (
    ...
    files_to_change_follow_up_prompt,
    pull_request_prompt,
    create_file_prompt,
...
class CodeGenBot(ChatGPT):
    ...
    def get_files_to_change(self):
        ...
        try:
            logger.info(f"Generating for the 1st and only time...")

            # First, generate abstract plan to help LLM understand what to do
            abstract_plan = self.chat(files_to_change_abstract_prompt, message_key="files_to_change")

            # Next, generate the actual plan
            ...
            try:
                ...
                # Failed to parse, so use CoT retry
                files_to_change_response = self.chat(files_to_change_follow_up_prompt, message_key="files_to_change")
                files_to_change = FilesToChange.from_string(files_to_change_response)

            create_thoughts = files_to_change.files_to_create.strip()
            modify_thoughts = files_to_change.files_to_modify.strip()

            # Generate file change requests
            ...
            logger.warning("Failed to parse with CoT!")
