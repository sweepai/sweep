...
- Did they use keyword searches to find all instances where a class, type, or function is used, not just where it is defined?
...
Examine each step taken by the contractor. Specifically call out anything done incorrectly or any files/usages that were missed. Provide a detailed explanation of the correct approach.
</judgement_on_task>
...
6-8: Good effort, but make sure to use keyword searches to find all usages of [specific class/function]. Don't forget to trace [specific dependency/import] as well.
1-5: The files you identified, such as [irrelevant files], are not actually relevant to this [specific issue]. Focus your search on [specific directory/file] instead. Use keyword searches to find all instances where [specific class/function] is used.
</message_to_contractor>"""

state_eval_prompt = """You are helping contractors on a task that involves finding all of the relevant files needed to resolve an issue. This task does not involve writing or modifying code. The contractors' goal is to identify all necessary files, not actually implement the solution. The contractor should not be coding. Be extremely critical and thorough in your evaluation.
...
They also used keyword searches for "Booking" to find the BookingController.java and BookingService.java files where Booking objects are created and processed. This shows they traced the usage and dependencies thoroughly.
...
<message_to_contractor>
Great work using keyword searches to identify all the files involved in the booking confirmation email flow, tracing the code from the core Booking model to the email service and HTML template!
</message_to_contractor>

Example 2 (Score: 5): 
<judgement_on_task>
The contractor identified the UserAccount.java file where the login bug is occurring, but failed to use keyword searches to find several other critical files. While they noted that the LoginController.java file calls the authenticateUser() method in UserAccount, they didn't search for "authenticateUser" to identify the LoginService.java file which is actually responsible for orchestrating the whole login flow.  
...
So while the contractor identified the core UserAccount class, they failed to use keyword searches to trace its dependencies and usages, missing several other key files that would likely need to be investigated and possibly modified to fully resolve the login issue.
</judgement_on_task>
<overall_score>5</overall_score>  
<message_to_contractor>
Use keyword searches for "UserAccount", "authenticateUser", "hash", etc to find all relevant files involved in the login process, not just the UserAccount definition, to improve.
</message_to_contractor>
...
Additionally, the contractor failed to look for any configuration files that provide pricing data, which could have been found by searching for "price" in JSON or properties files. By focusing solely on irrelevant front-end code and not using keyword searches to trace the actual pricing logic, the contractor demonstrated a complete lack of understanding of the actual bug and API architecture.
</judgement_on_task>
<overall_score>2</overall_score>  
<message_to_contractor>  
The front-end ProductList.vue file is not relevant for an API pricing bug. Keyword search for "price", "cost", "discount" etc to find the relevant backend code in the api/products/ directory instead.
</message_to_contractor>"""
...
class EvaluatorAgent(ChatGPT):
    def evaluate_run(self, problem_statement: str, run_text: str):
        ...
        formatted_problem_statement = f"This is the task for the contractor to research <task_to_research>\n{problem_statement}\n</task_to_research>"
