...
def enable_gha(sweep_bot: SweepBot, repo: Repository):
    try:
    ...
    if not config.get("gha_enabled"):
        config["gha_enabled"] = True
        sweep_bot.create_branch("gha-enable")
        sweep_bot.repo.update_file("sweep.yaml", "Enable GitHub Actions", yaml.safe_dump(config), contents.sha, branch="gha-enable")
        pr = sweep_bot.repo.create_pull(
            title="Enable GitHub Actions",
            body="This PR enables GitHub Actions for this repository by adding `gha_enabled: True` to `sweep.yaml`.",
            head="gha-enable",
            base=repo.default_branch,
        )
        pr.add_to_labels("sweep")
    # 2: Get human message
