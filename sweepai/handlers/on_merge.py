...
def on_merge(request_dict, chat_logger):
    posthog.capture('merge_start', {'merge_request': request_dict})
    if "commits" in request_dict and len(request_dict["commits"]) > 0:
        ...
        for commit in all_commits:
            ...
            posthog.capture('commit_identified', {'commit': commit})
    ...
    posthog.capture('files_changed', {'changed_files': changed_files})
    ...
    for file in changed_files:
        ...
        if issue_title:
            ...
            total_prs += 1
    posthog.capture('merge_end', {'total_prs': total_prs})
            total_prs += 1