...
def on_comment(
    ...
    try:
        ...
        if file_comment:
            ...
            if comment.strip().lower() == REVERT_BUTTON:
                file_path = comment.strip().split(" ")[1]
                sweep_bot.handle_revert_button(file_path)
                return {
                    "success": True,
                    "message": f"Reverted changes to {file_path}.",
                }
            elif comment.strip().lower() == REGENERATE_BUTTON:
                file_path = comment.strip().split(" ")[1]
                sweep_bot.handle_regenerate_button(file_path)
                return {
                    "success": True,
                    "message": f"Regenerated changes to {file_path}.",
                }
            else:
