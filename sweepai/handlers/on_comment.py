...
def on_comment(
    ...
    g = get_github_client(installation_id) if g is None else g
    repo = g.get_repo(repo_full_name) if repo is None else repo
    pr = repo.get_pull(pr_number) if pr is None else pr
