...
def on_ticket(
    ...
    try:
        ...
        payment_message = (
            f'{user_type}: I used {model_name} to create this ticket. You have {gpt_tickets_left_message}{daily_message}. (tracking ID: <a href="https://process.sweep.dev/issues/{tracking_id}"><code>{tracking_id}</code></a>)'
            + (purchase_message if not is_paying_user else "")
        )
        payment_message_start = (
            f'{user_type}: I\'m using {model_name}. You have {gpt_tickets_left_message}{daily_message}. (tracking ID: <a href="https://process.sweep.dev/issues/{tracking_id}"><code>{tracking_id}</code></a>)'
            + (purchase_message if not is_paying_user else "")
        )
