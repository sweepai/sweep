...
def on_ticket(
    ...
    try:
        ...
        payment_message, payment_message_start = create_payment_message(
            user_type=user_type, 
        ...
        def get_comment_header(
            ...
            return (
                ...
                + "\n"
                + center(create_payment_message(user_type, model_name, gpt_tickets_left_message, daily_message, purchase_message, is_paying_user, tracking_id)[1])
                + center(f"\n\n{markdown_badge}")
                + config_pr_message
                + f"\n\n---\n{actions_message}"
                + sandbox_execution_message
...
def create_payment_message(user_type, model_name, gpt_tickets_left_message, daily_message, purchase_message, is_paying_user, tracking_id):
    payment_message = (f"{user_type}: I used {model_name} to create this ticket. You have "
                       f"{gpt_tickets_left_message}{daily_message}. "
                       f"(tracking ID: <code>{tracking_id}</code>)"
                       + (purchase_message if not is_paying_user else ""))
    payment_message_start = (f"{user_type}: I'm using {model_name}. You have "
                             f"{gpt_tickets_left_message}{daily_message}. "
                             f"(tracking ID: <code>{tracking_id}</code>)"
                             + (purchase_message if not is_paying_user else ""))
