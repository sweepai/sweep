...
def on_ticket(
    ...
    try:
        ...
        issue_comment = None
        tickets_allocated, purchased_ticket_count = handle_payment_logic(is_consumer_tier, is_paying_user, chat_logger)
            chat_logger.get_ticket_count(purchased=True) if chat_logger else 0
        )
        ticket_count, daily_ticket_count, model_name, payment_link, single_payment_link = handle_payment_logic(tickets_allocated, purchased_ticket_count, chat_logger, use_faster_model)
        pro_payment_link, daily_message, user_type, gpt_tickets_left_message, purchase_message = handle_payment_logic(daily_ticket_count, is_paying_user, is_consumer_tier, ticket_count, single_payment_link)
...
from sweepai.utils.ticket_utils import (center, fetch_relevant_files,
                                        log_error, post_process_snippets)
...
def get_branch_diff_text(repo, branch):
    ...
    return "\n".join([f"{filename}\n{diff}" for filename, diff in pr_diffs])
            ...
            tickets_allocated, purchased_ticket_count, ticket_count, daily_ticket_count, model_name, payment_link, single_payment_link, pro_payment_link, daily_message, user_type, gpt_tickets_left_message, purchase_message, payment_message, payment_message_start = handle_payment_logic(is_consumer_tier, is_paying_user, chat_logger, use_faster_model)
            change_location = f" [`{pr_changes.pr_head}`](https://github.com/{repo_full_name}/commits/{pr_changes.pr_head}).\n\n"
            review_message = (
                "Here are my self-reviews of my changes at" + change_location
            )
...
def review_code(
    ...
    try:
        ...
        review_message += (
            f"Here is the {ordinal(1)} review\n" + blockquote(review_comment) + "\n\n"
def handle_payment_logic(is_consumer_tier, is_paying_user, chat_logger, use_faster_model):
    tickets_allocated = 5
    if is_consumer_tier:
        tickets_allocated = 15
    if is_paying_user:
    ...
    return tickets_allocated, purchased_ticket_count, ticket_count, daily_ticket_count, model_name, payment_link, single_payment_link, pro_payment_link, daily_message, user_type, gpt_tickets_left_message, purchase_message, payment_message, payment_message_start
        )
        if changes_required:
            edit_sweep_comment(
                review_message + "\n\nI'm currently addressing these suggestions.",
