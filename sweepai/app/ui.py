...
def main():
    ...
    **{title}**
    ...
    with gr.Blocks(theme=gr.themes.Soft(), title="Sweep Chat", css=css) as demo:
        ...
        with gr.Row():
            with gr.Column():
                plan_toggle = gr.Checkbox(label="Propose plan (Experimental)", value=lambda: global_state.plan_toggle,
                                          interactive=True, elem_id="plan_toggle")
            with gr.Column(scale=5):
                ...
                user_status = gr.Markdown(
                    value=f"Logged in as {config.github_username} ({tier_message}), {remaining_tickets} tickets remaining.")
        ...
        response = msg \
            ...
            .then(lambda: [gr.update(interactive=True), gr.update(interactive=True)], None, [msg, create_pr_button],
                  queue=False)
