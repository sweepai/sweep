...
def chunk(
    ...
    score: Union[float, List[float]] = 1.0,
    additional_metadata: Union[dict[str, str], List[dict[str, str]]] = {},
    max_chunk_size: int = 512 * 3,
    chunk_size: int = 10,
    overlap: int = 15,
):
    if isinstance(file_content, list):
        ...
        for i in range(len(file_content)):
            try:
                result = Chunking.chunk_core.call( # pylint: disable=no-member
                    file_content[i],
                    file_path[i],
                    score[i],
                ...
                # duplicate code
                logger.error(e)
                logger.info(file_path[i])
                ids = []
                metadatas = []
                logger.info("Unknown language")
                source_lines = file_content[i].split('\n')
                num_lines = len(source_lines)
                logger.info(f"Number of lines: {num_lines}")
                chunks = []
                start_line = 0
                while start_line < num_lines and num_lines > overlap:
                    ...
                    chunk = '\n'.join(source_lines[start_line:end_line])
                    chunks.append(chunk)
                    ids.append(f"{file_path[i]}:{start_line}:{end_line}")
                    metadatas.append({
                    ...
                    start_line += chunk_size - overlap
                results.append((chunks, metadatas, ids))
        return results
    else:
        try:
