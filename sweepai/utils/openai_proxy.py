...
class OpenAIProxy:
    ...
    def call_openai(self, model, messages, max_tokens, temperature) -> str:
        try:
            engine = None
            engine = self.determine_engine_and_handle_exclusivity(model, engine)
            if OPENAI_API_TYPE is None or engine is None:
                response = self.call_openai_with_default_settings(model, messages, max_tokens, temperature)
            ...
            if (
                ...
                response = self.call_openai_with_custom_engine(engine, model, messages, max_tokens, temperature)
                return response["choices"][0].message.content
    ...
    def determine_engine_and_handle_exclusivity(self, model, engine):
    ...
    def call_openai_with_custom_engine(self, engine, model, messages, max_tokens, temperature):
    ...
    def call_openai_with_default_settings(self, model, messages, max_tokens, temperature):
