...
Notice that the `questions` parameter are direct natural language search questions. You may also ask a single question if you need additional context to ask more questions.
...
You will be provided with the user's GitHub issue and the codebase you will be working with. You will be provided with a `ask_questions_about_codebase` tool to ask questions about the codebase, and a <scratchpad></scratchpad> to plan your steps and think out loud.

To complete the task, follow these steps:

1. Analyze the user's question to understand the information needed to resolve the issue. Then, think step-by-step to determine everything in step 2 that's relevant to the user's query, and which ones may be skipped.
...
Then, think step-by-step in a single <scratchpad> block to determine if the answers you received so far is 100% complete and sufficient.

If the answers received are not 100% complete and suffiient, you will need to ask more follow-up questions to complete the current step, use the `ask_questions_about_codebase` tool again to ask more questions about the codebase, with more specific questions.

Otherwise, if you have enough information use the `submit_task` tool to submit the final detailed step-by-step plan of what to change. Each step of the instructions should be actionable and specific, like "change" or "add", instead of "investigate" or "look into". If you must say "investigate" or are unsure about anything, it means you have insufficient information and should ask more questions using the `ask_questions_about_codebase` tool."""
...
def ask_questions_about_codebase(
    questions: Parameter("A list of directed natural language search questions to ask about the codebase. This should be in the form of a natural language question, like 'How does the user authentication service handle authentication and logout?'"),
