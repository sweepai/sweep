...
def recursive_hash(value, depth=0, ignore_params=[]):
    ...
    if isinstance(value, (int, float, str, bool, bytes)):
        ...
        return hashlib.md5(
            ''.join([recursive_hash(item, depth + 1, ignore_params) for item in value]).encode()).hexdigest()
    elif isinstance(value, dict):
        return hashlib.md5(''.join(
            [recursive_hash(key, depth + 1, ignore_params) + recursive_hash(val, depth + 1, ignore_params) for key, val
             in value.items() if key not in ignore_params]).encode()).hexdigest()
...
def file_cache(ignore_params=[]):
    ...
    def decorator(func):
        def wrapper(*args, **kwargs):
            ...
            arg_hash = recursive_hash(args_dict, ignore_params=ignore_params) + recursive_hash(kwargs,
